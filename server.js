...
app.post('/api/community/clubs',auth,featureAllowed('manage_clubs'),(req,res)=>{const u=current(req),name=cleanText(req.body.name).trim().slice(0,60);if(!name)return res.status(400).json({error:'Club name required.'});const c={id:id(),name,description:cleanText(req.body.description||'').slice(0,500),icon:String(req.body.icon||'🎭').slice(0,8),banner:String(req.body.banner||'').slice(0,500),ownerId:u.id,private:!!req.body.private,createdAt:Date.now()};db.clubs.push(c);db.clubMembers.push({id:id(),clubId:c.id,userId:u.id,role:'OWNER',time:Date.now()});addActivity('club',`${u.username} created ${name}`,u.id,{clubId:c.id});save();res.json({club:c});});
app.post('/api/community/clubs/:id/join',auth,(req,res)=>{const u=current(req),c=db.clubs.find(x=>x.id===req.params.id);if(!c)return res.status(404).json({error:'Club not found.'});if(c.private&&!hasFeature(u,'enter_private_club'))return res.status(403).json({error:'This club is private.'});if(!db.clubMembers.some(m=>m.clubId===c.id&&m.userId===u.id)){db.clubMembers.push({id:id(),clubId:c.id,userId:u.id,role:'MEMBER',time:Date.now()});grantRep(u,2,'joined a club');addActivity('club',`${u.username} joined ${c.name}`,u.id,{clubId:c.id});save();}res.json({ok:true,club:c});});
...
app.post('/api/community/mysteries',ownerOnly,(req,res)=>{const x={id:id(),title:cleanText(req.body.title||'Mystery').slice(0,100),intro:cleanText(req.body.intro||'').slice(0,1000),clues:Array.isArray(req.body.clues)?req.body.clues.map(cleanText).slice(0,20):[],answer:cleanText(req.body.answer||'').slice(0,200),endsAt:Number(req.body.endsAt)||Date.now()+7*86400000,time:Date.now()};db.mysteries.push(x);save();res.json({mystery:{...x,answer:undefined}});});
...
function malReply(u,text){const q=String(text||'').trim();db.malAI ||= {enabled:true,commands:[]};if(db.malAI.enabled){const custom=db.malAI.commands.find(x=>x.enabled!==false&&q.toLowerCase()===('@mal '+x.command).toLowerCase());if(custom)return custom.response;}if(/^@Mal\s+Time\s*$/i.test(q)){const d=new Date(),t=d.toLocaleTimeString('en-IN',{hour12:false,timeZone:'Asia/Kolkata'});return `${u.username} : ${t}`;}if(/^@Mal\s+Help\s*$/i.test(q))return 'Mal commands: @Mal Time · @Mal Room · @Mal Users · @Mal Event · @Mal Rules · @Mal Fortune · @Mal Secret';if(/^@Mal\s+Users\s*$/i.test(q))return `There are ${db.users.filter(x=>x.online).length} users online.`;if(/^@Mal\s+Room\s*$/i.test(q))return 'You are in the current room. Use the room panel for its full details.';if(/^@Mal\s+Fortune\s*$/i.test(q)){const pool=['Keep creating.','Be curious today.','A good conversation is worth having.'];return pool[Math.floor(Math.random()*pool.length)]}if(/^@Mal\s+Event\s*$/i.test(q)){const e=(db.communityEvents||[]).filter(x=>!x.end||x.end>Date.now()).sort((a,b)=>(a.start||0)-(b.start||0))[0];return e?`Next event: ${e.title} — ${e.start?new Date(e.start).toLocaleString('en-IN'):''}`:'No upcoming event.';}if(/^@Mal\s+Rules\s*$/i.test(q))return 'Please follow the room rules, respect other members, and follow staff instructions.';if(/^@Mal\s+Secret\s*$/i.test(q))return 'Try discovering hidden commands around Maleficent Chat.';return null;}
...
const communityInterval=setInterval(()=>{
  ...
  for(const item of db.scheduledMessages){
    if(item.status==="scheduled"&&Number(item.sendAt)<=now){
      const u=db.users.find(x=>x.id===item.userId),r=db.rooms.find(x=>x.id===item.roomId);
      if(u&&r){
        const m={id:id(),roomId:r.id,userId:u.id,username:u.username,displayName:u.displayName,rank:u.rank,text:item.text,reactions:{},deleted:false,pinned:false,time:now,scheduled:true,delivered:true};
        db.messages.push(m);
        io.to("room:"+r.id).emit("message",m);
        io.to("user:"+u.id).emit("scheduled:sent",{scheduledId:item.id,message:m});
        notify(u.id,"message","Scheduled message sent",`Your scheduled message was sent in ${r.name}.`);
      }
