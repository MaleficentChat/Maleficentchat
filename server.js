...
const DEFAULT_GIFTS=[
  ...
  {id:"teddy",name:"Teddy Bear",icon:"🧸",cost:200,creator:"SYSTEM",custom:false},
  {id:"sparkles",name:"Sparkles",icon:"✨",cost:75,creator:"SYSTEM",custom:false}
];
if(!Array.isArray(db.gifts) || db.gifts.length===0) db.gifts=DEFAULT_GIFTS.map(x=>({...x}));
db.malAI ||= {enabled:true,commands:[]};
db.clubMessages ||= [];
...
const FEATURE_CONTROLS=[
  ...
  {key:"manage_aurora",name:"Manage Aurora",category:"AI",defaultRank:"OWNER"},
  {key:"manage_diaval",name:"Manage Diaval",category:"AI",defaultRank:"OWNER"},
  ...
  {key:"manage_club_members",name:"Manage club members",category:"Community",defaultRank:"ADMIN"},
  {key:"manage_club_settings",name:"Manage club settings",category:"Community",defaultRank:"ADMIN"},
  {key:"create_puzzles",name:"Create puzzle rooms",category:"Community",defaultRank:"ADMIN"},
  {key:"create_mysteries",name:"Create mystery events",category:"Community",defaultRank:"OWNER"},
  {key:"manage_hidden_secrets",name:"Manage hidden secrets",category:"Community",defaultRank:"OWNER"},
...
app.post('/api/community/upload',auth,upload.single("file"),(req,res)=>{
  if(!req.file) return res.status(400).json({error:"No file selected."});
  if(!/^(image\/|audio\/|video\/)/i.test(req.file.mimetype||"")) return res.status(400).json({error:"Only image, audio or video files are allowed."});
  res.json({ok:true,url:"/uploads/"+req.file.filename,name:req.file.originalname,type:req.file.mimetype,size:req.file.size});
});
...
app.post(
  ...
  (req,res)=>{
    ...
    const malTimeMatch = /^@Mal\s+/i.test(text);
    ...
    if(matchedFilter && (rankOrder[u.rank]??999)<=filterMaxRank){
      ...
      notify(u.id,"moderation","Auto-mute",`Your message matched the room filter and you were muted for ${minutes} minute${minutes===1?"":"s"}.`); systemNotice(`@${u.username} has been muted automatically for ${minutes} minute${minutes===1?"":"s"}.`,r.id);
    }
...
function systemNotice(text, roomId=null){
  const cfg=db.auroraConfig||{};if(cfg.enabled===false||cfg.systemActions===false)return null;
  const targetRoom=roomId||cfg.actionRoomId||null;
  const notice={id:id(),system:true,username:'Aurora',displayName:'Aurora',rank:'BOT',text:String(text),roomId:targetRoom,time:Date.now(),reactions:{},pinned:false,deleted:false,auroraAction:true};
  if(targetRoom){db.messages.push(notice);save();io.to('room:'+targetRoom).emit('message',notice);}else{io.emit('system-message',notice);}return notice;
}

function diavalLocalReply(question, roomId){
  const cfg=db.diavalConfig||{};if(cfg.enabled===false)return "Diaval is currently disabled by the owner.";if(roomId&&cfg.roomMentions===false)return null;
  const q=String(question||'').trim();const custom=(cfg.customCommands||[]).find(x=>x.enabled!==false&&q.toLowerCase()===String(x.command||'').toLowerCase());if(custom)return custom.response;
  ...
  const recent=cfg.conversationContext===false?[]:db.messages.filter(m=>m.roomId===roomId&&!m.deleted&&!m.system).slice(-6).map(m=>String(m.text||'').trim()).filter(Boolean);
...
function auroraReplyFor(text){
  const cfg=db.auroraConfig||{};if(cfg.enabled===false||cfg.truthDare===false)return null;
...
app.post('/api/diaval/message',auth,(req,res)=>{
  ...
  const maxPairs=Math.max(1,Math.min(100,Number(cfg.maxMemory||30))); if(u.diavalHistory.length>maxPairs*2)u.diavalHistory=u.diavalHistory.slice(-(maxPairs*2));
...
if(db.malAI && db.malAI.legacyCleaned!==true){ db.malAI.commands=[]; db.malAI.legacyCleaned=true; db.malAI.enabled=true; }
db.polls ||= [];
db.pollVotes ||= [];
db.siteThemes ||= [];
db.watchQueues ||= {};
...
db.clubMembers ||= [];
db.clubMessages ||= [];
db.auroraConfig ||= {enabled:true,systemActions:true,truthDare:true,actionRoomId:"main",announceAutomations:true};
db.diavalConfig ||= {enabled:true,roomMentions:true,directChat:true,calculations:true,conversationContext:true,customCommands:[],maxMemory:30,responseMode:"helpful"};
db.malAI ||= {enabled:true,commands:[],legacyCleaned:false};
db.userNotes ||= {};
db.userLabels ||= {};
...
app.get("/api/community/scheduled",auth,(req,res)=>{const u=current(req);res.json({scheduled:(db.scheduledMessages||[]).filter(x=>x.userId===u.id).sort((a,b)=>Number(a.sendAt)-Number(b.sendAt))});});
...
/* ===================== 3.3 SOCIAL PLATFORM UPGRADE ===================== */
for(const k of [
  'clubs','clubMembers','profilePresets','artworks','moments','musicQueues','puzzles','mysteries','secrets','activityFeed','dailyFortunes','hallOfFame','archives','featureLab','locales'
]) db[k] ||= [];
db.featureLab ||= {};
db.locales ||= {};
function addActivity(type,text,userId=null,meta={}){
  db.activityFeed.push({id:id(),type,text,userId,time:Date.now(),meta});
  if(db.activityFeed.length>1000) db.activityFeed=db.activityFeed.slice(-1000);
}
function reputation(u){return Number(u.reputation||0)}
function repLevel(n){return n>=1000?'Legend':n>=500?'Veteran':n>=200?'Trusted':n>=50?'Regular':'Newcomer'}
function grantRep(u,n,reason){u.reputation=reputation(u)+n;u.reputationLevel=repLevel(u.reputation);addActivity('reputation',`${u.username} gained ${n} reputation${reason?' · '+reason:''}`,u.id);}

app.get('/api/community/platform',auth,(req,res)=>{
  const u=current(req), now=Date.now();
  res.json({
    activeRooms:db.rooms.map(r=>({...r,passwordHash:undefined,onlineCount:io.sockets.adapter.rooms.get('room:'+r.id)?.size||0})).sort((a,b)=>b.onlineCount-a.onlineCount),
    upcomingEvents:(db.communityEvents||[]).filter(e=>!e.end||e.end>=now).slice(0,8),
    reputation:{points:reputation(u),level:repLevel(reputation(u))},
    featureLab:db.featureLab||{},
    season:(db.seasons||[]).find(x=>x.active&&(!x.endsAt||x.endsAt>now))||null,
    activity:db.activityFeed.slice(-12).reverse(),
    suggestedRooms:db.rooms.filter(r=>r.id!==u.roomId).slice(0,6),
    dailyFortune:(db.dailyFortunes.find(x=>x.userId===u.id&&x.day===new Date().toISOString().slice(0,10))||null)?.text||null
  });
});

app.get('/api/community/clubs',auth,(req,res)=>res.json({clubs:db.clubs.map(c=>({...c,members:db.clubMembers.filter(m=>m.clubId===c.id).length}))}));
app.post('/api/community/clubs',auth,featureAllowed('manage_clubs'),(req,res)=>{const u=current(req),name=cleanText(req.body.name).trim().slice(0,60);if(!name)return res.status(400).json({error:'Club name required.'});const c={id:id(),name,description:cleanText(req.body.description||'').slice(0,500),icon:String(req.body.icon||'🎭').slice(0,8),banner:String(req.body.banner||'').slice(0,500),ownerId:u.id,private:!!req.body.private,passwordHash:req.body.password?bcrypt.hashSync(String(req.body.password),10):null,rankRequired:req.body.rankRequired||'MEMBER',rules:cleanText(req.body.rules||'').slice(0,2000),createdAt:Date.now()};db.clubs.push(c);db.clubMembers.push({id:id(),clubId:c.id,userId:u.id,role:'OWNER',time:Date.now()});addActivity('club',`${u.username} created ${name}`,u.id,{clubId:c.id});save();res.json({club:{...c,passwordHash:undefined}});});
app.post('/api/community/clubs/:id/join',auth,async(req,res)=>{const u=current(req),c=db.clubs.find(x=>x.id===req.params.id);if(!c)return res.status(404).json({error:'Club not found.'});if(c.rankRequired&&!hasRank(u,c.rankRequired))return res.status(403).json({error:`This club requires ${c.rankRequired} or higher.`});if(c.private&&!hasFeature(u,'enter_private_club'))return res.status(403).json({error:'This club is private.'});if(c.passwordHash&&!(await bcrypt.compare(String(req.body.password||''),c.passwordHash)))return res.status(403).json({error:'Incorrect club password.'});if(!db.clubMembers.some(m=>m.clubId===c.id&&m.userId===u.id)){db.clubMembers.push({id:id(),clubId:c.id,userId:u.id,role:'MEMBER',time:Date.now()});grantRep(u,2,'joined a club');addActivity('club',`${u.username} joined ${c.name}`,u.id,{clubId:c.id});save();}res.json({ok:true,club:{...c,passwordHash:undefined}});});
app.get('/api/community/clubs/:id',auth,(req,res)=>{const c=db.clubs.find(x=>x.id===req.params.id);if(!c)return res.status(404).json({error:'Club not found.'});const joined=db.clubMembers.some(m=>m.clubId===c.id&&m.userId===current(req).id);if(c.private&&!joined&&!hasFeature(current(req),'manage_clubs'))return res.status(403).json({error:'Join this private club first.'});res.json({club:c,joined,members:db.clubMembers.filter(m=>m.clubId===c.id).map(m=>{const u=findUser(m.userId);return {...m,user:u?safeUser(u):null}}),messages:db.clubMessages.filter(x=>x.clubId===c.id).slice(-100)});});
app.patch('/api/community/clubs/:id',auth,featureAllowed('manage_club_settings'),(req,res)=>{const u=current(req),c=db.clubs.find(x=>x.id===req.params.id);if(!c)return res.status(404).json({error:'Club not found.'});if(c.ownerId!==u.id&&!hasRank(u,'ADMIN'))return res.status(403).json({error:'Club owner or admin required.'});for(const k of ['name','description','icon','banner','rules','rankRequired'])if(req.body[k]!==undefined)c[k]=cleanText(req.body[k]).slice(0,k==='rules'?2000:500);if(req.body.private!==undefined)c.private=!!req.body.private;if(req.body.password!==undefined)c.passwordHash=req.body.password?bcrypt.hashSync(String(req.body.password),10):null;save();res.json({club:{...c,passwordHash:undefined}});});
app.delete('/api/community/clubs/:id',auth,featureAllowed('manage_clubs'),(req,res)=>{const u=current(req),c=db.clubs.find(x=>x.id===req.params.id);if(!c)return res.status(404).json({error:'Club not found.'});if(c.ownerId!==u.id&&!hasRank(u,'ADMIN'))return res.status(403).json({error:'Club owner or admin required.'});db.clubs=db.clubs.filter(x=>x.id!==c.id);db.clubMembers=db.clubMembers.filter(x=>x.clubId!==c.id);db.clubMessages=db.clubMessages.filter(x=>x.clubId!==c.id);save();res.json({ok:true});});
app.patch('/api/community/clubs/:id/members/:userId',auth,featureAllowed('manage_club_members'),(req,res)=>{const u=current(req),c=db.clubs.find(x=>x.id===req.params.id),m=db.clubMembers.find(x=>x.clubId===req.params.id&&x.userId===req.params.userId);if(!c||!m)return res.status(404).json({error:'Club or member not found.'});if(c.ownerId!==u.id&&!hasRank(u,'ADMIN'))return res.status(403).json({error:'Club owner or admin required.'});if(req.body.role&&['MEMBER','MOD','ADMIN'].includes(req.body.role))m.role=req.body.role;if(req.body.remove)db.clubMembers=db.clubMembers.filter(x=>x!==m);save();res.json({ok:true});});
app.post('/api/community/clubs/:id/messages',auth,(req,res)=>{const u=current(req),c=db.clubs.find(x=>x.id===req.params.id);if(!c)return res.status(404).json({error:'Club not found.'});if(!db.clubMembers.some(m=>m.clubId===c.id&&m.userId===u.id))return res.status(403).json({error:'Join the club before posting.'});const text=cleanText(req.body.text||'').trim().slice(0,2000);if(!text)return res.status(400).json({error:'Message cannot be empty.'});const m={id:id(),clubId:c.id,userId:u.id,username:u.username,displayName:u.displayName,rank:u.rank,text,time:Date.now()};db.clubMessages.push(m);if(db.clubMessages.length>5000)db.clubMessages=db.clubMessages.slice(-5000);addActivity('club',`${u.username} posted in ${c.name}`,u.id,{clubId:c.id});save();res.json({message:m});});

app.get('/api/community/reputation/:id?',auth,(req,res)=>{const u=req.params.id?findUser(req.params.id):current(req);if(!u)return res.status(404).json({error:'User not found.'});res.json({points:reputation(u),level:repLevel(reputation(u))});});
app.post('/api/community/reputation/grant',auth,(req,res)=>{const actor=current(req);if(!hasRank(actor,'MOD'))return res.status(403).json({error:'Moderator required.'});const u=findUser(req.body.userId);const n=Math.max(-50,Math.min(50,Math.floor(Number(req.body.amount)||0)));if(!u||!n)return res.status(400).json({error:'Invalid reputation change.'});grantRep(u,n,cleanText(req.body.reason||'').slice(0,120));save();res.json({user:safeUser(u),points:reputation(u),level:repLevel(reputation(u))});});

app.get('/api/community/presets',auth,(req,res)=>res.json({presets:db.profilePresets.filter(x=>x.userId===current(req).id)}));
app.post('/api/community/presets',auth,(req,res)=>{const u=current(req),p={id:id(),userId:u.id,name:cleanText(req.body.name||'Preset').slice(0,40),avatar:cleanText(req.body.avatar||'').slice(0,500),banner:cleanText(req.body.banner||'').slice(0,500),frame:cleanText(req.body.frame||'').slice(0,100),nameplate:cleanText(req.body.nameplate||'').slice(0,100),bio:cleanText(req.body.bio||'').slice(0,500),theme:cleanText(req.body.theme||'').slice(0,40)};db.profilePresets.push(p);save();res.json({preset:p});});
app.delete('/api/community/presets/:id',auth,(req,res)=>{db.profilePresets=db.profilePresets.filter(x=>!(x.id===req.params.id&&x.userId===current(req).id));save();res.json({ok:true});});

app.get('/api/community/gallery',auth,(req,res)=>res.json({artworks:db.artworks.slice().reverse()}));
app.post('/api/community/gallery',auth,(req,res)=>{const u=current(req),a={id:id(),userId:u.id,username:u.username,title:cleanText(req.body.title||'Untitled').slice(0,100),description:cleanText(req.body.description||'').slice(0,500),url:cleanText(req.body.url||'').slice(0,1000),category:cleanText(req.body.category||'Other').slice(0,40),likes:[],time:Date.now()};if(!a.url)return res.status(400).json({error:'Artwork URL required.'});db.artworks.push(a);grantRep(u,3,'published artwork');save();res.json({artwork:a});});
app.post('/api/community/gallery/:id/like',auth,(req,res)=>{const a=db.artworks.find(x=>x.id===req.params.id);if(!a)return res.status(404).json({error:'Artwork not found.'});const u=current(req);a.likes ||= [];const i=a.likes.indexOf(u.id);if(i>=0)a.likes.splice(i,1);else a.likes.push(u.id);save();res.json({likes:a.likes.length,liked:i<0});});

app.get('/api/community/moments',auth,(req,res)=>{const now=Date.now();res.json({moments:db.moments.filter(x=>x.expiresAt>now).slice().reverse()});});
app.post('/api/community/moments',auth,(req,res)=>{const u=current(req),mins=Math.max(1,Math.min(10080,Number(req.body.minutes)||1440)),m={id:id(),userId:u.id,username:u.username,text:cleanText(req.body.text||'').slice(0,500),media:cleanText(req.body.media||'').slice(0,1000),expiresAt:Date.now()+mins*60000,time:Date.now()};db.moments.push(m);save();res.json({moment:m});});

app.get('/api/community/music/:roomId',auth,(req,res)=>res.json({queue:db.musicQueues.find(x=>x.roomId===req.params.roomId)||{roomId:req.params.roomId,items:[],current:null}}));
app.post('/api/community/music/:roomId',auth,(req,res)=>{let q=db.musicQueues.find(x=>x.roomId===req.params.roomId);if(!q){q={roomId:req.params.roomId,items:[],votes:{}};db.musicQueues.push(q)}const item={id:id(),title:cleanText(req.body.title||'').slice(0,120),url:cleanText(req.body.url||'').slice(0,1000),user:current(req).username,votes:0};if(!item.title)return res.status(400).json({error:'Track title required.'});q.items.push(item);save();res.json({queue:q});});
app.post('/api/community/music/:roomId/vote/:itemId',auth,(req,res)=>{const q=db.musicQueues.find(x=>x.roomId===req.params.roomId),it=q?.items.find(x=>x.id===req.params.itemId);if(!it)return res.status(404).json({error:'Track not found.'});q.votes ||= {};q.votes[it.id] ||= [];const u=current(req);const i=q.votes[it.id].indexOf(u.id);if(i>=0)q.votes[it.id].splice(i,1);else q.votes[it.id].push(u.id);it.votes=q.votes[it.id].length;save();res.json({queue:q});});

app.get('/api/community/puzzles',auth,(req,res)=>res.json({puzzles:db.puzzles.map(x=>({...x,answer:undefined}))}));
app.post('/api/community/puzzles',auth,featureAllowed('create_puzzles'),(req,res)=>{const u=current(req),x={id:id(),title:cleanText(req.body.title||'Puzzle').slice(0,80),question:cleanText(req.body.question||'').slice(0,500),answer:cleanText(req.body.answer||'').trim().toLowerCase(),reward:{xp:10,gold:25},creator:u.username,time:Date.now()};if(!x.question||!x.answer)return res.status(400).json({error:'Puzzle question and answer required.'});db.puzzles.push(x);save();res.json({puzzle:{...x,answer:undefined}});});
app.post('/api/community/puzzles/:id/solve',auth,(req,res)=>{const x=db.puzzles.find(x=>x.id===req.params.id),u=current(req);if(!x)return res.status(404).json({error:'Puzzle not found.'});const correct=cleanText(req.body.answer||'').trim().toLowerCase()===x.answer;if(correct){u.xp=(u.xp||0)+10;u.gold=(u.gold||0)+25;grantRep(u,5,'solved a puzzle');save()}res.json({correct,reward:correct?x.reward:null});});

app.get('/api/community/mysteries',auth,(req,res)=>res.json({mysteries:db.mysteries.filter(x=>!x.endsAt||x.endsAt>Date.now()).map(x=>({...x,answer:undefined}))}));
app.post('/api/community/mysteries',auth,featureAllowed('create_mysteries'),(req,res)=>{const x={id:id(),title:cleanText(req.body.title||'Mystery').slice(0,100),intro:cleanText(req.body.intro||'').slice(0,1000),clues:Array.isArray(req.body.clues)?req.body.clues.map(cleanText).slice(0,20):[],answer:cleanText(req.body.answer||'').slice(0,200),endsAt:Number(req.body.endsAt)||Date.now()+7*86400000,time:Date.now()};db.mysteries.push(x);save();res.json({mystery:{...x,answer:undefined}});});
app.post('/api/community/mysteries/:id/solve',auth,(req,res)=>{const x=db.mysteries.find(x=>x.id===req.params.id);if(!x)return res.status(404).json({error:'Mystery not found.'});const correct=cleanText(req.body.answer||'').trim().toLowerCase()===x.answer.trim().toLowerCase();if(correct){const u=current(req);u.gold=(u.gold||0)+100;u.xp=(u.xp||0)+50;grantRep(u,20,'solved a mystery');save()}res.json({correct,reward:correct?{gold:100,xp:50}:null});});

app.get('/api/community/secrets',auth,(req,res)=>{const u=current(req);u.discoveredSecrets ||= [];res.json({secrets:db.secrets.map(x=>({...x,found:u.discoveredSecrets.includes(x.id)}))});});
app.post('/api/community/secrets/:id/discover',auth,(req,res)=>{const x=db.secrets.find(x=>x.id===req.params.id),u=current(req);if(!x)return res.status(404).json({error:'Secret not found.'});u.discoveredSecrets ||= [];if(!u.discoveredSecrets.includes(x.id)){u.discoveredSecrets.push(x.id);u.gold=(u.gold||0)+(x.reward?.gold||10);grantRep(u,5,'discovered a secret');save()}res.json({found:true,reward:x.reward||{gold:10}});});
app.post('/api/owner/secrets',auth,featureAllowed('manage_hidden_secrets'),(req,res)=>{const x={id:id(),command:cleanText(req.body.command||'/secret').slice(0,40),title:cleanText(req.body.title||'Hidden Secret').slice(0,100),text:cleanText(req.body.text||'').slice(0,500),reward:{gold:Math.max(0,Number(req.body.gold)||10)},time:Date.now()};db.secrets.push(x);save();res.json({secret:x});});

app.get('/api/community/activity',auth,(req,res)=>res.json({activity:db.activityFeed.slice(-100).reverse()}));
app.get('/api/community/fortune',auth,(req,res)=>{const u=current(req),day=new Date().toISOString().slice(0,10);let f=db.dailyFortunes.find(x=>x.userId===u.id&&x.day===day);if(!f){const pool=['A small act of kindness may brighten your day.','Try something creative today.','A good conversation can become a good memory.','Take a moment to notice something beautiful.','Today is a good day to learn something new.'];f={id:id(),userId:u.id,day,text:pool[Math.floor(Math.random()*pool.length)]};db.dailyFortunes.push(f);save()}res.json({fortune:f});});
app.get('/api/community/hall',auth,(req,res)=>{const users=db.users.map(u=>({username:u.username,displayName:u.displayName,reputation:reputation(u),messages:u.stats?.messages||0,artworks:db.artworks.filter(a=>a.userId===u.id).length,games:u.gameStats?.played||0,joined:u.createdAt}));res.json({reputation:users.slice().sort((a,b)=>b.reputation-a.reputation).slice(0,10),messages:users.slice().sort((a,b)=>b.messages-a.messages).slice(0,10),art:users.slice().sort((a,b)=>b.artworks-a.artworks).slice(0,10),veterans:users.slice().sort((a,b)=>a.joined-b.joined).slice(0,10)});});
app.get('/api/community/archive',auth,(req,res)=>res.json({news:(db.news||[]).slice().reverse(),events:(db.communityEvents||[]).slice().reverse(),museum:(db.museum||[]).slice().reverse(),memories:(db.communityMemories||[]).slice().reverse()}));

app.get('/api/owner/feature-lab',ownerOnly,(req,res)=>res.json({features:db.featureLab}));
app.patch('/api/owner/feature-lab/:key',ownerOnly,(req,res)=>{db.featureLab[req.params.key]=!!req.body.enabled;save();res.json({ok:true,key:req.params.key,enabled:db.featureLab[req.params.key]});});
app.put('/api/owner/feature-lab',ownerOnly,(req,res)=>{ensureFeaturePermissions();const allowed=['clubs','communityMap','identityPresets','profileShowcase','artGallery','moments','musicRooms','puzzleRooms','mysteryEvents','easterEggs','liveActivity','dailyFortune','hallOfFame','archive','localization','malCommands'];db.featureLab ||= {};for(const k of allowed){if(req.body[k]!==undefined)db.featureLab[k]=!!req.body[k];}save();res.json({ok:true,features:db.featureLab});});
app.get('/api/owner/aurora',ownerOnly,(req,res)=>{db.auroraConfig ||= {enabled:true,systemActions:true,truthDare:true,actionRoomId:"main",announceAutomations:true};res.json({config:db.auroraConfig,truthCount:(db.aurora?.truth||[]).length,dareCount:(db.aurora?.dare||[]).length});});
app.patch('/api/owner/aurora',ownerOnly,(req,res)=>{db.auroraConfig ||= {};for(const k of ['enabled','systemActions','truthDare','announceAutomations'])if(req.body[k]!==undefined)db.auroraConfig[k]=!!req.body[k];if(req.body.actionRoomId!==undefined)db.auroraConfig.actionRoomId=String(req.body.actionRoomId||'main');save();res.json({ok:true,config:db.auroraConfig});});
app.get('/api/owner/diaval',ownerOnly,(req,res)=>{db.diavalConfig ||= {enabled:true,roomMentions:true,directChat:true,calculations:true,conversationContext:true,customCommands:[],maxMemory:30,responseMode:"helpful"};res.json({config:db.diavalConfig});});
app.patch('/api/owner/diaval',ownerOnly,(req,res)=>{db.diavalConfig ||= {};for(const k of ['enabled','roomMentions','directChat','calculations','conversationContext'])if(req.body[k]!==undefined)db.diavalConfig[k]=!!req.body[k];if(req.body.maxMemory!==undefined)db.diavalConfig.maxMemory=Math.max(0,Math.min(100,Number(req.body.maxMemory)||30));if(req.body.responseMode!==undefined&&['helpful','concise','creative','royal'].includes(String(req.body.responseMode)))db.diavalConfig.responseMode=String(req.body.responseMode);if(Array.isArray(req.body.customCommands))db.diavalConfig.customCommands=req.body.customCommands.map(x=>({id:x.id||id(),command:cleanText(x.command||'').trim().slice(0,60),response:cleanText(x.response||'').slice(0,1000),enabled:x.enabled!==false})).filter(x=>x.command&&x.response).slice(0,100);save();res.json({ok:true,config:db.diavalConfig});});
app.get('/api/owner/mal-ai',ownerOnly,(req,res)=>{db.malAI ||= {enabled:true,commands:[]};res.json({enabled:db.malAI.enabled,commands:db.malAI.commands});});
app.put('/api/owner/mal-ai',ownerOnly,(req,res)=>{db.malAI ||= {enabled:true,commands:[]};db.malAI.enabled=req.body.enabled!==false;save();res.json({ok:true,enabled:db.malAI.enabled,commands:db.malAI.commands});});
app.post('/api/owner/mal-ai/commands',ownerOnly,(req,res)=>{db.malAI ||= {enabled:true,commands:[]};const command=cleanText(req.body.command||'').trim().replace(/^@Mal\s*/i,'').slice(0,40);const response=cleanText(req.body.response||'').trim().slice(0,1000);if(!command||!response)return res.status(400).json({error:'Command and response are required.'});if(!/^[A-Za-z0-9_-]+$/.test(command))return res.status(400).json({error:'Use letters, numbers, hyphens or underscores for the command.'});const existing=db.malAI.commands.find(x=>x.command.toLowerCase()===command.toLowerCase());if(existing)existing.response=response;else db.malAI.commands.push({id:id(),command,response,enabled:true,createdAt:Date.now(),updatedAt:Date.now()});save();res.json({ok:true,commands:db.malAI.commands});});
app.patch('/api/owner/mal-ai/commands/:id',ownerOnly,(req,res)=>{const x=(db.malAI?.commands||[]).find(x=>x.id===req.params.id);if(!x)return res.status(404).json({error:'Command not found.'});if(req.body.command!==undefined)x.command=cleanText(req.body.command).trim().replace(/^@Mal\s*/i,'').slice(0,40);if(req.body.response!==undefined)x.response=cleanText(req.body.response).trim().slice(0,1000);if(req.body.enabled!==undefined)x.enabled=!!req.body.enabled;x.updatedAt=Date.now();save();res.json({ok:true,command:x});});
app.delete('/api/owner/mal-ai/commands/:id',ownerOnly,(req,res)=>{if(!db.malAI)return res.json({ok:true});db.malAI.commands=db.malAI.commands.filter(x=>x.id!==req.params.id);save();res.json({ok:true,commands:db.malAI.commands});});
app.get('/api/owner/dev-console',ownerOnly,(req,res)=>res.json({uptime:process.uptime(),memory:process.memoryUsage(),users:db.users.length,online:db.users.filter(x=>x.online).length,rooms:db.rooms.length,messages:db.messages.length,notifications:db.notifications.length,dbBytes:fs.existsSync(DATA)?fs.statSync(DATA).size:0,node:process.version,featureLab:db.featureLab}));

app.get('/api/locales',auth,(req,res)=>res.json({locale:current(req).settings?.locale||'en',available:{en:'English',mr:'मराठी',hi:'हिन्दी',es:'Español',fr:'Français',de:'Deutsch'}}));
app.patch('/api/locales',auth,(req,res)=>{const u=current(req),allowed=['en','mr','hi','es','fr','de'];if(!allowed.includes(req.body.locale))return res.status(400).json({error:'Unsupported language.'});u.settings ||= {};u.settings.locale=req.body.locale;save();res.json({locale:req.body.locale});});

// Mal local assistant commands used in chat. No external API is required.
function malReply(u,text){
  const q=String(text||'').trim();db.malAI ||= {enabled:true,commands:[],legacyCleaned:true};
  if(!db.malAI.enabled)return null;
  const custom=(db.malAI.commands||[]).find(x=>x.enabled!==false&&q.toLowerCase()===('@mal '+x.command).toLowerCase());
  if(custom)return custom.response;
  if(/^@Mal\s+Time\s*$/i.test(q)){const d=new Date(),t=d.toLocaleTimeString('en-IN',{hour12:false,timeZone:'Asia/Kolkata'});return `${u.username} : ${t}`;}
  return null;
}


const communityInterval=setInterval(processScheduledMessages,1000);
communityInterval.unref?.();


app.get("/health", (req,res)=>res.json({ok:true,status:"online",service:"Maleficent Chat",version:"3.6.0"}));
