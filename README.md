...
## 3.2 Community Lab additions

- Message threads and thread replies.
- Scheduled room messages and expiring messages.
- Bookmark folders and personal profile labels.
- User notes, profile guestbooks, profile walls and activity timelines.
- Feature suggestion box with voting and staff status workflow.
- Mini blogs and a site changelog.
- Community reputation, collectible collections and trading cards.
- Sticker library and soundboard.
- Configurable room bots and room lore.
- Chat Time Machine for date-based room history.
- Community Memories, Seasons and Museum.
- User-created quizzes with XP/Gold rewards.
- Interactive welcome-tour entry point.
- Community Lab inside the existing Community Hub.

### Mal bot

The chat now includes a lightweight local bot named `Mal`. In any room, send exactly:

`@Mal Time`

Mal replies using the sender's username and the server's current local time, for example:

`User A : 19:30:00`

No external AI/API credits are required for this command.


## 3.6 deployment fixes
- The project files are at the repository/service root so Render can find `package.json`.
- Added `GET /health` for Render/service health checks.
- Pinned Render to Node 20.19.0 for a predictable runtime.
- Removed stray archive/download artifacts from the deploy package.
