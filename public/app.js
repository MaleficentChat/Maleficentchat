...
function setupMentionAutocomplete(){
  ...
  input.addEventListener('input', async ()=>{
    ...
    try{
      ...
      const bots=[{id:'bot-aurora',username:'Aurora',displayName:'Aurora',rank:'BOT',online:true,avatar:''},{id:'bot-diaval',username:'Diaval',displayName:'Diaval',rank:'BOT',online:true,avatar:''}];
      state.mentionUsers=[...bots,...liveUsers].slice(0,10);
      if(!state.mentionUsers.length){box.classList.remove('show');return;}
      box.innerHTML=state.mentionUsers.map(u=>`<button type="button" data-mention="${esc(u.username)}">${u.rank==='BOT'?`<span class="bot-mention-icon">${u.username==='Aurora'?'🪄':'🐉'}</span>`:avatar(u)}<span><b>${esc(u.displayName)}</b><small>@${esc(u.username)} · ${u.rank==='BOT'?'AI bot':'Online'}</small></span></button>`).join('');
...
$("notifications").onclick=async()=>{
  try{
    ...
    modal(`<h2>Notifications</h2>${(data.notifications||[]).map(n=>`<div class="card"><b>${esc(n.title)}</b><br>${esc(n.text)}<br><small>${fmt(n.time)}</small></div>`).join("")||'<p class="muted">No notifications.</p>'}`);
    await api("/api/notifications/read",{method:"POST"}); await loadNotifications();
  }catch(e){toast(e.message)}
};
...
async function playRPS(){const choices=['Rock','Paper','Scissors'];const mine=choices[Math.floor(Math.random()*3)],bot=choices[Math.floor(Math.random()*3)];const result=mine===bot?'draw':((mine==='Rock'&&bot==='Scissors')||(mine==='Paper'&&bot==='Rock')||(mine==='Scissors'&&bot==='Paper'))?'win':'loss';const label=result==='draw'?'Draw!':result==='win'?'You win!':'Diaval wins!';const html=`<b>✊ You:</b> ${mine} · <b>🤖 Diaval:</b> ${bot}<br><strong>${label}</strong>`;const el=$('gameResult')||$('usersGameResult');if(el)el.innerHTML=html;await recordGame('rps',result);}
async function rollDice(){const a=1+Math.floor(Math.random()*6),b=1+Math.floor(Math.random()*6);const result=a===b?'draw':a>b?'win':'loss';const html=`🎲 You rolled <b>${a}</b> · Diaval rolled <b>${b}</b> — <strong>${result==='draw'?'Draw!':result==='win'?'You win!':'Diaval wins!'}</strong>`;const el=$('gameResult')||$('usersGameResult');if(el)el.innerHTML=html;await recordGame('dice',result);}
